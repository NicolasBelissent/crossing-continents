...
// Send notification to all subscribers about a new video
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    const { request, env } = context;
    const { title, description, url, secret } = await request.json() as VideoNotification;
    ...
    const videoEmail = emailTemplates.newVideo(title, description, url, '');
    let successCount = 0;
    let failCount = 0;
        ...
        batch.map(async (email) => {
          try {
            await resend.emails.send({
              ...
              to: email,
              subject: videoEmail.subject,
              html: addUnsubscribeUrl(videoEmail.html, email),
            });
            successCount++;
          } catch (error) {
